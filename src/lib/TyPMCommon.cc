...
static unordered_map<string, set<StringRef>> elementsStructNameMap;
...
string getFileName(const DILocation *Loc, const DISubprogram *SP) {
	string FN;
	if (Loc)
...
string getSourceFuncName(const Instruction *I) {
...
string extractMacro(string line, const Instruction *I) {
...
const DILocation *getSourceLocation(const Instruction *I) {
	if (!I)
		return NULL;

	auto *N = I->getMetadata("dbg");
	if (!N)
		return NULL;
...
void printSourceCodeInfo(const Value *V, string Tag) {
...
void WriteSourceInfoIntoFile(const Value *V, string file_name) {
	auto *I = dyn_cast<Instruction>(V);
	if (!I)
		return;
	auto *Loc = getSourceLocation(I);
...
void printSourceCodeInfo(const Function *F, string Tag) {
...
void WriteSourceInfoIntoFile(const Function *F, string file_name) {
	DISubprogram *SP = F->getSubprogram();
	string srcInfo = "";
	std::ofstream oFile;
	oFile.open(file_name, std::ios::out | std::ios::app);
...
string getMacroInfo(const Value *V) {
...
string funcTypeString(const FunctionType *FTy) {
