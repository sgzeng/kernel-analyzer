#ifndef _TYPM_COMMON_H_
#define _TYPM_COMMON_H_
...
string getFileName(const DILocation *Loc,
		const DISubprogram *SP=NULL);
...
void printSourceCodeInfo(const Value *V, string Tag = "VALUE");
void printSourceCodeInfo(const Function *F, string Tag = "FUNC");
...
size_t moduleTypeHash(const Module *M, size_t TyH);
