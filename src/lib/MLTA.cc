...
#define MLTA_LOG(stmt) KA_LOG(2, "MLTA: " << stmt)
#define MLTA_DEBUG(stmt) KA_LOG(3, "MLTA: " << stmt)
...
// Implementation
//
inline MLTA::typeidx_t MLTA::typeidx_c(Type *Ty, int Idx) {
	return make_pair(Ty, Idx);
}
inline MLTA::hashidx_t MLTA::hashidx_c(size_t Hash, int Idx) {
...
void MLTA::findCalleesWithType(const CallInst *CI, FuncSet &S) {
	...
	for (auto *F : Ctx->AddressTakenFuncs) {
		...
		auto *CalleeM = F->getParent();
		auto *CallerM = CI->getFunction()->getParent();
...
Type *MLTA::getFuncPtrType(const Value *V) {
	Type *Ty = V->getType();
...
const Value *MLTA::recoverBaseType(const Value *V) {
	if (auto I = dyn_cast<Instruction>(V)) {
		auto &AliasMap = AliasStructPtrMap[I->getFunction()];
...
const BasicBlock* MLTA::getParentBlock(const Value* V) {
...
Type* MLTA::getRealType(const Value *V) {

	auto BB = getParentBlock(V);
	if(!BB)
		return NULL;
	...
	for(auto i = BB->begin(); i != BB->end(); i++){
		...
		if(CI){
			...
			auto cai_first_arg = CI->getArgOperand(0);
			...
			auto cai_second_arg = CI->getArgOperand(1);

			if (auto MDAV = dyn_cast<MetadataAsValue>(cai_second_arg)) {

				auto MD = MDAV->getMetadata();
				if(!MD)
					continue;
				...
				auto DILV = dyn_cast<DILocalVariable>(N);
				if(!DILV)
					continue;

				auto DITy = DILV->getType();
				if(!DITy)
					continue;

				auto currentDITy = DITy;
				...
				while (true) {
					...
					auto DIDTy = dyn_cast<DIDerivedType>(currentDITy);
					if (DIDTy) {
						currentDITy = DIDTy->getBaseType();
						continue;
					}

					auto DIBTy = dyn_cast<DIBasicType>(currentDITy);
					if (DIBTy) {
						break;
					}

					auto DICTy = dyn_cast<DICompositeType>(currentDITy);
					if (DICTy) {
...
bool MLTA::typeConfineInInitializer(const GlobalVariable *GV) {
	...
	while (!LU.empty()) {
		...
		for (auto oi = U->op_begin(), oe = U->op_end();
			...
			// field of another composite-type object
				...
				auto F = dyn_cast<Function>(PIO->getOperand(0));
				if (F)
					FoundF = F;
				else {
					auto OU = dyn_cast<User>(PIO->getOperand(0));
					LU.push_back(OU);
				}
			}
...
// The analysis is field sensitive.
bool MLTA::typeConfineInFunction(const Function *F) {
	...
	for (auto i = inst_begin(F), e = inst_end(F); i != e; ++i) {
		...
		if (auto SI = dyn_cast<StoreInst>(I)) {
			...
			for (auto OI = I->op_begin(), OE = I->op_end();
					OI != OE; ++OI) {
				if (auto F = dyn_cast<Function>(*OI)) {
					if (F->isIntrinsic())
						continue;
...
void MLTA::collectAliasStructPtr(const Function *F) {

	auto &AliasMap = AliasStructPtrMap[F];
...
void MLTA::escapeType(const Value *V) {

	typelist_t TyChain;
	bool Complete = true;
	getBaseTypeChain(TyChain, V, Complete);
	for (auto T : TyChain) {
		MLTA_DEBUG("[Escape] Type: " << *(T.first) << "; Idx: " << T.second << "\n");
		typeEscapeSet.insert(typeIdxHash(T.first, T.second));
	}
}

void MLTA::confineTargetFunction(const Value *V, const Function *F) {
	if (F->isIntrinsic())
		return;
	...
	getBaseTypeChain(TyChain, V, Complete);
	for (auto TI : TyChain) {
		MLTA_DEBUG("[INSERT-FUNC] Container type: " << *(TI.first) << "; Idex: "
		    << TI.second << "\n\t --> FUNC:  " << F->getName() << "; Module: "
			<< F->getParent()->getName() << "\n");
		MLTA_DEBUG("[HASH] " << typeHash(TI.first) << "\n");
...
void MLTA::propagateType(const Value *ToV, Type *FromTy, int Idx) {
...
const Value *MLTA::getVTable(const Value *V) {
...
void MLTA::printTypeChain(typelist_t &Chain) {
	if (Chain.empty())
		return;
...
// propagate further
bool MLTA::getBaseTypeChain(typelist_t &Chain, const Value *V,
		bool &Complete) {

	Complete = true;
	const Value *CV = V, *NextV = NULL;
	typelist_t TyList;
	visited_t Visited;
...
Type *MLTA::getBaseType(const Value *V, visited_t &Visited) {
	...
	// The value itself is a pointer to a composite type
		...
		else if (auto BV = recoverBaseType(V)) {
...
Type *MLTA::_getPhiBaseType(const PHINode *PN, visited_t &Visited) {
...
bool MLTA::getGEPLayerTypes(const GEPOperator *GEP, typelist_t &TyList) {
	...
	if (ConstI && ConstI->getSExtValue() != 0) {
		...
		if (StructType *STy = dyn_cast<StructType>(ETy)) {
			...
			for (auto User : GEP->users()) {
				if (auto BCO = dyn_cast<BitCastOperator>(User)) {
					OptGEP = true;
#ifdef SOUND_MODE
	...
	if (Indices.empty()) {
		for (auto it = GEP->idx_begin(); it != GEP->idx_end(); it++) {
			auto ConstI = dyn_cast<ConstantInt>(it->get());
			if (ConstI)
...
bool MLTA::nextLayerBaseTypeWL(const Value *V, typelist_t &TyList,
		const Value* &NextV) {

	deque<const Value*> VL;
	VL.push_back(V);
	visited_t Visited;
	...
	while (!VL.empty()) {
		...
		else if (auto LI = dyn_cast<LoadInst>(CV)) {

			NextV = LI->getPointerOperand();
			VL.push_back(LI->getOperand(0));
		}
		else if (auto BCO = dyn_cast<BitCastOperator>(CV)) {
		...
		else if (auto SelI = dyn_cast<SelectInst>(CV)) {
		...
		else if (auto UO = dyn_cast<UnaryOperator>(CV)) {
...
const Function *MLTA::getBaseFunction(const Value *V) {
...
bool MLTA::findCalleesWithMLTA(const CallInst *CI,
	...
	while (ContinueNextLayer) {
		...
		for (auto TyIdx : TyList) {
			...
			MLTA_DEBUG("[CONTAINER] Type: " << *(TyIdx.first)
				<< "; Idx: " << TyIdx.second << "\n");
			MLTA_DEBUG("[HASH] " << typeHash(TyIdx.first) << "\n");
